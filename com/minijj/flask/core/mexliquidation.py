...
class mexliquidation(object):
    def __init__(self, mex,server):
        ...
        self.buyqueue = Queue.Queue(20)
        self.sellqueue = Queue.Queue(20)
        self.conn = Conn_db()
    ...
    def buyThread(self,order):
        ...
        while 1:
            ...
            try:
                if order[2] > 0:
                    logger.info(
                        '###mex买入建仓### price= ' + bytes(order[1]) + " slipp = " + bytes(slipp) + " amount= " + bytes(
                            abs(order[2]) * 100 - partdel))
                else:
                ...
                if rst['ordStatus'] and 'Filled' == rst['ordStatus']:
                    ...
                    if order[2] > 0:
                        logger.info("#####okcoin=" + bytes(order[0]) + " #####mex=" + bytes(
                            rst['avgPx']) + "##amount=" + bytes(order[2]) + " ##basic_create=" + bytes(
                            order[4]) + "建仓滑点 " + bytes(
                        ...
                        his.append((datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 1, order[0], rst['avgPx'],
                                    order[2], order[4], order[4] - order[0] + rst['avgPx']))
                        self.conn.set(constants.trade_his_key, his)

                    else:
                        logger.info("#####okcoin=" + bytes(order[0]) + " #####mex=" + bytes(
                            rst['avgPx']) + "##amount=" + bytes(order[2]) + " ##basic_create=" + bytes(
                            order[4]) + "平仓滑点 " + bytes(order[3] + order[4] - order[0] + rst['avgPx']) + " #######")
                        his = self.conn.get(constants.trade_his_key)
                        his.append((datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 1, order[0], rst['avgPx'],
                    ...
                    if 'Filled' == cel[0]['ordStatus']:
                        ...
                        if order[2] > 0:
                            logger.info("#####okcoin=" + bytes(order[0]) + " #####mex=" + bytes(
                                cel[0]['avgPx']) + "##amount=" + bytes(order[2]) + " ##basic_create=" + bytes(
                                order[4]) + "建仓以持仓变化均价建仓滑点 " + bytes(
                            ...
                            logger.info("#####okcoin=" + bytes(order[0]) + " #####mex=" + bytes(
                                cel[0]['avgPx']) + "##amount=" + bytes(order[2]) + " ##basic_create=" + bytes(
                                order[4]) + "平仓滑点 " + bytes(
                                order[3] + order[4] - order[0] + cel[0]['avgPx']) + " #######")
    ...
    def sellThread(self,order):
        ...
        while 1:
            ...
            try:
                ...
                if order[2] > 0:
                    logger.info(
                        '###mex卖出开仓### price= ' + bytes(order[1]) + " slipp = " + bytes(slipp) + " amount= " + bytes(
                            abs(order[2]) * 100 - partdel))
                else:
                    logger.info(
                        '###mex买入平仓### price= ' + bytes(order[1]) + " slipp = " + bytes(slipp) + " amount= " + bytes(
                            abs(order[2]) * 100 - partdel))
                ...
                if rst['ordStatus'] and 'Filled' == rst['ordStatus']:
                    ...
                    if order[2] > 0:
                        logger.info("#####okcoin=" + bytes(order[0]) + " #####mex=" + bytes(
                            rst['avgPx']) + "##amount=" + bytes(order[2]) + " ##liquid_bais=" + bytes(
                            order[4]) + "建仓滑点 " + bytes(
                        ...
                        logger.info(
                            "#####okcoin=" + bytes(order[0]) + " #####mex=" + bytes(rst['avgPx']) + "##amount=" + bytes(
                                order[2]) + " ##liquid_bais=" + bytes(order[4]) + "平仓滑点 " + bytes(
                                order[3] - order[4] + order[0] - rst['avgPx']) + " #######")
                        his = self.conn.get(constants.trade_his_key)
                        his.append(
                    ...
                    if 'Filled' == cel[0]['ordStatus']:  # 已经成交无法取消了，哈哈哈
                        ...
                        if order[2] > 0:
                            logger.info("#####okcoin=" + bytes(order[0]) + " #####mex=" + bytes(
                                cel[0]['avgPx']) + "##amount=" + bytes(order[2]) + " ##liquid_bais=" + bytes(
                                order[4]) + "建仓滑点 " + bytes(
